...
**曾按用户授权在官网生成真实待支付订单，未付款；尚未证明全自动抢稀缺票。** 当次最终确认由浏览器接管完成，旧版自动回查未完成。目前通过 160 项自动化测试（包含 5 个离线 Chromium 登录场景）；生产适配器另有 9 个离线 Chromium 页面场景及完整断网浏览器下单/回查/重启防重测试，混合模式也通过了生产解析器与浏览器的合成全流程。新回查逻辑仍缺一次真实订单的完整验证，本轮接口重构没有提交真实订单。不能把测试通过、点击成功或有票时下单等同于抢票成功率提升。[原可行性调研](docs/feasibility.md)和[接口可行性实测](docs/api-feasibility.md)记录了证据和限制。
...
会话失效跳回登录页后，控制台等待该页脚本就绪后切换扫码，不再空等“退出”入口直到页面超时。二维码图片加载完成才提示“请扫码”；加载失败会停止并给出重试提示。等待扫码时刷新控制台会自动重新连接官方浏览器窗口。
...
接口查票与只读监控独立运行 HTTP 客户端，不要求启动 Chromium 或扫码；登录、页面查询及预订时才启动浏览器。Docker 重建留下的旧容器浏览器锁会在确认无活动浏览器且取得配置目录排他锁后清理，只处理失效锁链接，保留原 Cookie 和配置。查询失败会在车次列表内显示原因，部分日期失败时保留已完成日期的结果。

首次初始化时，从当前官网页面读取查询入口，不固定猜测 `queryG/queryU` 等地址。后续同一会话直接取余票 JSON，按官网当前字段含义解析车次、席别、库存与价格；不加载查询页面，不逐车次请求票价。独立客户端从专用 Cookie 检查点恢复会话；浏览器启动后交接 Cookie，并改用浏览器共享的请求上下文。令牌和原始余票行不输出到日志。接口出错遵循原有退避，不自动切换引擎掩盖错误。
